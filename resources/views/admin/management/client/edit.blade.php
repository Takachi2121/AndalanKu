...
@section('content')
<form action="{{ route('updateClient', $data->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="mb-4">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <input type="text" value="{{ $data->nama_client }}" class="form-control" id="nama_client" name="nama_client" placeholder="Contoh: Kemnaker" required>
            </div>
        </div>
    </div>
    ...
    <div class="mb-4">
        ...
        <div class="col-md-6">
            ...
            <!-- Preview Gambar -->
            <div class="mt-3">
                <img id="preview-thumbnail" src="{{ $data->logo_client ? asset('img/data/client/' . $data->logo_client) : '#' }}"
                alt="Preview Gambar Client"
                style="max-width: 220px; max-height: 220px; {{ $data->logo_client ? '' : 'display: none;' }} border: 1px solid #ddd; padding: 5px; border-radius: 4px;">
            </div>
        </div>
    </div>
